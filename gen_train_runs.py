...
for env_name in sorted(all_environments):
    for exp_num in range(num_experiments):
        for algo_name, replay_size in experiment_configs:
            print(f". /opt/conda/etc/profile.d/conda.sh && conda activate && python experiment_train.py {env_name} {algo_name} --replay_buffer_size={replay_size} --frames={num_frames} --experiment-seed={exp_num}")
